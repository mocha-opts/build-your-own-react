...
第一行代码**定义了一个 React 元素**。

第二行代码从 DOM 中 **获取了 id 为 root 的元素**。

第三行代码将 **定义的 React 元素** 渲染到 **获取到的元素中**去。
...
**React.createElement** 方法通过传入的参数创建一个对象, 我们也可以简单模仿一下 React.createElement 创建的对象。

我们所定义的元素是一个具有两个属性（type 和 props）的对象（当然真正的 ReactElement 不止这两个属性，不过在这里我们只关心这两个）。

type 指的是我们想要创建的 DOM 元素类型的字符串，等同于通过 document.createElement 方法来创建 DOM 元素时传入的 tagName。type 也可以是一个函数，但是为了不造成困惑，这个等到第七步时我们再说。

props 也是一个对象，它存放的是 JSX 属性的键和值，还有一个特殊的属性 **children**。
...
还需要我们来处理的一个方法就是 ReactDOM.render。

render 是 React 来改变 DOM 的方法，这样简单的一句描述有些过于宽泛了，不过我们先可以根据这点来试着捣鼓捣鼓它是怎么改变 DOM 的。
...
同时需要注意，我们在为节点设置 nodeValue 属性的这个过程，和设置节点 h1 设置 title 属性类似，就像是在设置一个文本节点的 nodeValue 为 Hello，即 `props: {nodeValue: "hello"}`。
...
到此为止，我们就完成了用原生的 JS 来替代 JSX 语法这一目标
...
现在,我们要创建一个自己的 React 来替代真正的 React

就从实现 `createElement` 方法开始吧

首先，在 JSX 转换到 JS 的这个过程中，让我们看看` createElement` 这个方法到底都做了些什么
...
在前面我们提到过，一个元素（我们简化的）实际上就是一个带有 type 和 props 这两个属性的对象。

我们的方法唯一需要思考的就是如何去创建这个对象。
...
在代码中，我们使用展开操作符... 来处理 children，这样，children 就变成了一个数组。
...
官方的 `React` 并没有封装原始类型的值,也没有在 没有 children 时创建一个空数组.我们这样做是为了简化代码，因为我们的目标是理解，而不是追求高性能
...
如果我们有这样的注释，当 Babel 转义 JSX 时，它将使用我们定义的函数。
...
接着，我们来关注最后一行代码。下面的目标就是实现我们自己的 render 方法以替换 ReactDOM.render。
...
先搭好了架子，接着就只需要在 render 方法内部去实现具体的功能就好了。
...
等等，别忘了文本元素. 如果元素是`TEXT_ELEMENT`类型 ，我们要为它创建文本节点。
...
最后，节点有了，就该为它添加`props` 属性了。记得需要排除 `props` 中的 `children`。
...
现在我们的库 Didact 也可以将 JSX 元素渲染成真正的 DOM 节点了。

你也可以在 [codesandbox](https://codesandbox.io/s/didact-2-k6rbj?file=/src/index.js:0-1113) 上看看最终的效果怎么样。
...
注意以上代码，因为采用的是递归，一旦我们开始渲染，在整个元素树渲染的过程是无法停下来的。

如果元素树非常复杂的话，就会占用主线程非常多的时间，从而造成阻塞。

试着想一下，如果浏览器需要处理优先级更高的事件如用户输入、保持动画流畅运行，这时浏览器就不得不等到 render 方法执行完后再来处理这些事情。

既然递归会造成这么严重的问题，那我们之前为什么要采用递归的方式呢？ 我猜是因为 React 之前也是采用不可中断的递归的方式。😈
...
首先，我们来将整个任务拆分成一个个小的任务单元 work unit，然后在每次执行完一个小的任务单元后询问一下浏览器有没有其他需要优先处理的事情。

这个过程我们用`requestIdleCallback`浏览器原生方法来实现, 可以理解为`setTimeout`,但有差异.

当主线程空闲时，浏览器将运行回调，而不是告诉我们什么时候运行。

---

```js focus=47:49,53,55:56,58

```

requestIdlecallback 还为我们提供了一个截止日期参数。可以用它来检查到浏览器需要再次控制为止还有多少时间。
...
在开始这个循环之前，我们需要先指定第一个任务单元，因此我们创建一个 `performUnitOfWork` 方法.

在这个方法中需要去执行这个任务，同时最后返回下一个需要执行的任务单元 `nextUnitOfWork`。
...
为了组织任务单元，我们需要一种数据结构：fiber tree。
...
![0](https://pomb.us/static/de664d437c94d478778b965c66c91f99/d3fa7/fiber0.png)

在 render 方法中，我们先创建 `root fiber` 并把它作为第一个任务单元，即第一个 `nextUnitOfWork`。

其余的任务会在 `performUnitOfWork` 这个方法中去处理。

对于每个 fiber 我们需要在 performUnitOfWork 这个方法中做三件事情：
...
![1](https://pomb.us/static/a88a3ec01855349c14302f6da28e2b0c/d3fa7/fiber1.png)

当我们执行完一个 fiber 的任务时，如果它的 child 指针指向的元素不为空的话，则该元素将会被指定为 `nextUnitOfWork`。

以上图中的 fiber tree 为例，当我们执行完 div 的 fiber 的任务时，`nextUnitOfWork` 指针就会指向 h1 元素 的 fiber。

如果一个 fiber 的 child 指针指向的元素为空，即该 fiber 不存在 child : **我们将会指定 sibling 对应的 fiber 作为 `nextUnitOfWork`**。

**如果一个 fiber 即没有 child 也没有 sibling**， `nextUnitOfWork` 指向为这个 fiber 的 uncle，也就是这个 fiber 的父节点 parent 的兄弟节点 sibling。

上图 fiber tree 中标签为 a 的 fiber 就没有 child 和 sibling，于是向上寻找最后找到了它的叔叔，标签为 h2 的 fiber。

如果该 fiber 没有叔叔呢？我们就会继续沿着 parent 向上寻找，直到找到一个存在 sibling 的 parent 或者最终到 `根 root`。如果是后者的话，那就说明我们已经执行完了本次渲染 render 的所有任务。

---

接着，就让我们把想法转换为代码吧。

在开始之前，我们先将 render 方法中之前的代码移除
...
并将移除的那部分放到方法 createDom 中去，这将会在之后用到
...
首先，在 render 方法中将 nextUnitOfWork 初始化为 fiber tree 的根。
...
在 performUnitOfWork 中，我们要先为传入的 fiber 创建对应的 DOM 节点。

首先，我们创建一个新的节点并将其附加到 DOM,并且我们会使用 fiber.dom 属性来定位该 DOM 节点。
...
每次我们在处理一个元素时都需要将其新建的节点添加到 DOM 上去。

但是不要忘了，浏览器能够在我们渲染完整棵 fiber tree 之前中断我们的渲染过程。

出现这种情况时，用户将会看到一个不完整的 UI 界面，那是我们不想看到的。
...
移除后我们换种方法,首先保存对 fiber tree 的根 root 的引用,将其引用命名为 `wipRoot` （work in progress root）。
...
只有处理完了所有任务单元（可以根据 nextUnitOfWork 是否为空来判断），我们才把整棵 fiber tree 提交到 DOM 上。
...
这就是下面我们需要实现的目标,为此我们要把本次渲染的 `fiber tree` 即 `wipRoot` 和 上一次提交到 DOM 上的 `fiber tree` 进行比较。

---

那么为了方便下次比较，在每次提交阶段完成时，我们就需要保存对本次提交到 DOM 上的 fiber tree 的引用，其引用命名为 `currentRoot`。同时，我们也为每个 fiber 添加 alternate 属性，指向旧的 fiber，即在上个提交阶段我们添加到 DOM 上的 fiber。
...
接着，让我们从 performUnitOfWork 方法中抽离创建新的 fiber 那部分代码，原代码：
...
将抽离出来的方法放到 reconcileChildren 方法中，改造后的代码：
...
在 reconcileChildren 这个方法中，我们将对旧的 fiber 与新的元素进行调和 reconclie,也就是常说的 diff 算法。
...
我们同时迭代旧 fiber (`wipFiber.alternate`) 的子元素 以及` elements` 数组中我们想要调和的元素。

如果我们忽略同时迭代链表和数组所需要注意的那些通用代码，只关心其中最重要的部分：

oldFiber 和 element，element 是本次调和我们想要渲染到 DOM 上的 fiber，oldFiber 是我们上次渲染到 DOM 上的 fiber。我们需要比较它们的差异。
...
1. 如果 oldFiber 和 element 的 type 一样，我们可以保留其 DOM 节点，只更新其中的属性 props。
2. 如果它们的 type 不一样：

- 新的 element 不在 oldFiber 集合当中，我们需要创建新的 DOM 节点。
- oldFiber 存在于 新的 element 集合中 ， 但对应的 element 不同则不能直接复用和更新，需要移除旧的 DOM 节点。
...
同时我们为新的 fiber 新增了 effectTag 属性。这个属性我们将会在后面的`commit`阶段用到。
...
当需要创建一个新的 DOM 节点的情况， effectTag 就会设置为 PLACEMENT。
...
当我们需要删除对应的节点的时候。因为不会创建新的 fiber，所以我们在旧的 fiber 上设置 effectTag 为 DELETION。
...
这样，我们就实现了更新和删除，让我们在 [codesandbox](https://codesandbox.io/s/didact-6-96533) 上看看加入了调和的 Didact 效果怎么样。
...
这是我们从示例中调用计数器函数的地方。在该功能中，我们称为 useState。
...
我们需要在调用函数组件之前初始化一些全局变量，以便我们可以在 useState 函数内使用它们。
...
可以在[codesandbox](https://codesandbox.io/s/didact-8-21ost)上体验完整的代码
